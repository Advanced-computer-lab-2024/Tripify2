...
export default function ProductPage({ product, cart }) {
  ...
  const handlePurchase = async () => {
    ...
    try
    {
      const response = await fetcher(`/bookings/products/create-booking/${product._id}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
    ...
    <div className="container px-4 py-8 mx-auto">
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <div>
          ...
          <p className="mb-4 text-2xl font-bold">{currency === 'USD' ? '$' : currency === 'EUR' ? '€' : 'EGP'}{convertPrice(product.Price, currency)}</p>
          ...
          <div className="flex items-center mb-6">
            <Input
              ...
              className="w-20 mr-4"
            />
            <Button onClick={handlePurchase} disabled={loading} className="flex items-center transition-all duration-200 ease-in-out">
          ...
          <div className="flex items-center mb-6 space-x-4">
            ...
            >
              {WishList2.includes(product._id) ? "♥ Added to Wishlist" : "♡ Add to Wishlist"}
            </Button>
          </div>
          <p className="mb-4 text-sm text-gray-500">
            {product.AvailableQuantity} items available | {product.TotalSales} sold
          </p>
                ...
                <CardContent className="p-4">
                  <div className="flex items-start">
                    <Avatar className="w-10 h-10 mr-4">
                      <AvatarImage src={`https://api.dicebear.com/6.x/initials/svg?seed=${review.UserId.UserName}`} />
                      <AvatarFallback>{review.UserId.UserName.charAt(0)}</AvatarFallback>
                    </Avatar>
                    <div className="flex-grow">
                      <div className="flex items-center justify-between mb-2">
                        <div>
                          <p className="font-semibold">{review.UserId.UserName}</p>
                          <p className="text-sm text-gray-500">{review.UserId.Role}</p>
                        </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          ...
          <p>Thank you for your purchase of {quantity} {product.Name}(s).</p>
