...
export default function ItineraryDetails({ itinerary }) {
  ...
  const handleShareEmail = () => {
    ...
    const body = encodeURIComponent(`I found this amazing itinerary and thought you might be interested:\n\n${itinerary.Name}\n\nCheck it out here: ${window.location.href}`)
    ...
    <div className="max-w-6xl p-6 mx-auto bg-gray-50">
      <div className="overflow-hidden bg-white rounded-lg shadow-lg">
        <div className="relative h-96">
          <img src={itinerary.Image} alt={itinerary.Name} className="object-cover w-full h-full" />
          <div className="absolute inset-0 bg-gradient-to-t from-black to-transparent"></div>
          <div className="absolute bottom-0 left-0 right-0 p-6">
            <h1 className="mb-2 text-4xl font-bold text-white">{itinerary.Name}</h1>
            {itinerary.Tag.map((tag, index) => (
              <Badge key={index} variant='neutral' className='text-black bg-white'>
                {tag.Tag}
              </Badge>
            ))}
          </div>
        ...
        <div className="flex justify-end mb-4 space-x-4">
          ...
          <div className="grid grid-cols-1 gap-6 mb-8 md:grid-cols-3">
            ...
            <div className="flex items-center space-x-3">
              ...
              <span className="text-gray-700">Tour Guide: {itinerary.TourGuide.YearsOfExperience} years exp.</span>
            </div>
            <div className="flex items-center space-x-3">
              <ClockIcon className="w-6 h-6 text-yellow-500" />
              <span className="text-gray-700">Duration: {itinerary.Activities.reduce((acc, act) => acc + act.duration, 0)} minutes</span>
            </div>
            <div className="flex items-center space-x-3">
              <CheckIcon className="w-6 h-6 text-purple-500" />
              <span className="text-gray-700">Accessibility: {itinerary.Accesibility ? 'Yes' : 'No'}</span>
            </div>
            <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center space-x-3">
              ...
              <span className="text-gray-700">{itinerary.RemainingBookings} spots left</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-blue-600">{currency === 'USD' ? '$' : currency === 'EUR' ? '€' : 'EGP'} {convertPrice(itinerary.Price * numParticipants, currency)}</p>
            </div>
            <button
              onClick={handleBook}
