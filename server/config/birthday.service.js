...
class BirthdayService {
    ...
    async checkAndSendBirthdayPromoCodes() {
        try {
            ...
            console.log(`Found ${birthdayTourists.length} tourists with birthdays today`);
    ...
    async createBirthdayPromoAndNotify(tourist) {
        try {
            const promoCode = `BDAY${tourist.UserId._id.toString().slice(-6)}${new Date().getFullYear()}`;
            ...
            console.log(`Birthday promo code ${promoCode} created and sent to tourist ${tourist.UserName}`);

        } catch (error) {
            console.error(`Error processing birthday promotion for tourist ${tourist.UserName}:`, error);
        }
    }
}
