...
const createActivity = async (req, res) => {
    const { Date, Time, Location, Price, CategoryId, Tags, SpecialDiscounts,AdvertiserId, Duration} = req.body;

    try {
        const newActivity = await new activityModel({ Date, Time, Location, Price, CategoryId, Tags, SpecialDiscounts,AdvertiserId, Duration });
        await newActivity.save();
        res.status(201).json({ message: 'Advertiser created successfully', activity: newActivity });
    } catch (error) {
...
const updateActivity = async (req, res) => {
    ...
    try {
        const updatedActivity = await activityModel.findByIdAndUpdate(id, { Date, Time, Location, Price, CategoryId, Tags, SpecialDiscounts,AdvertiserId, Duration }, { new: true });
...
module.exports = { createActivity, getActivity, getActivityById, updateActivity, deleteActivity };
